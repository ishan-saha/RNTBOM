...
const Sidebar = ({ mobileOpen, onCloseMobile }) => {
    ...
    const navItem = (to, Icon, label) => {
        ...
        return (
            ...
            >
                ...
                <span className={`${desktopExpanded ? "md:inline" : "md:hidden"} inline`}>
                    {label}
                </span>
            </Link>
        );
    ...
    return (
        <>
            ...
            <aside
                className={`fixed inset-y-0 left-0 z-50 bg-[#0b0b14] border-r border-white/10 flex flex-col transition-transform duration-300 md:sticky md:top-0 md:translate-x-0 md:h-screen ${desktopExpanded ? "md:w-64" : "md:w-20"
            ...
            >
                ...
                <div className="flex items-center justify-between p-4">
                    <h1 className={`text-white font-semibold ${desktopExpanded ? "md:inline" : "md:hidden"} inline`}>
                        SBOM
                    </h1>
                ...
                <div className="px-4 py-3 border-t border-white/10 border-b border-white/10">
                    ...
                    <Link to="/profile" onClick={onCloseMobile} className="flex items-center gap-3 rounded-lg p-1 hover:bg-white/5 transition-colors">
                        <div className="w-8 h-8 bg-indigo-600 rounded-full flex items-center justify-center text-white font-bold">
                            {user?.name?.charAt(0).toUpperCase()}
                        </div>
                        <div className={`${desktopExpanded ? "md:block" : "md:hidden"} block min-w-0`}>
