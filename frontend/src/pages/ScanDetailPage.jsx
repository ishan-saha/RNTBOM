...
const SeverityBadge = ({ severity }) => {
  ...
  return (
    <span
      className={`text-xs px-2 py-0.5 rounded border font-medium capitalize ${cls}`}
    >
      {severity || "unknown"}
    </span>
  );
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span
      className={`flex items-center gap-1 text-xs px-2 py-0.5 rounded border font-medium capitalize ${cls}`}
    >
      {icon}
      {status}
    </span>
...
const ScanDetailPage = () => {
  ...
  const TH =
    "px-3 py-2 text-left text-xs font-semibold text-white bg-[#2b2bb2] border border-[#1f1f8a]";
  const TD = "px-3 py-2 text-xs text-gray-800 border border-gray-200";
  const TDalt =
    "px-3 py-2 text-xs text-gray-800 border border-gray-200 bg-gray-50";
    ...
    <div className="p-6 md:p-10 bg-[#0f0f1a] min-h-screen text-white">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          {
            label: "Duration",
            value: getDuration(scan.startedAt, scan.completedAt),
          ...
          {
            label: "Components",
            value: scan.componentCount ?? 0,
            icon: <Package className="w-4 h-4 text-purple-400" />,
          },
        ].map(({ label, value, icon }) => (
          <div
            key={label}
            className="bg-[#13131f] border border-white/10 rounded-xl p-4"
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          {
            label: "Critical",
            value: scan.vulnCritical,
            cls: "text-red-400 border-red-500/20 bg-red-500/5",
          },
          {
            label: "High",
            value: scan.vulnHigh,
            cls: "text-orange-400 border-orange-500/20 bg-orange-500/5",
          },
          {
            label: "Medium",
            value: scan.vulnMedium,
            cls: "text-yellow-400 border-yellow-500/20 bg-yellow-500/5",
          },
          {
            label: "Low",
            value: scan.vulnLow,
            cls: "text-blue-400 border-blue-500/20 bg-blue-500/5",
          },
        ].map(({ label, value, cls }) => (
                    ...
                    <tbody>
                      {components.map((c, i) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-indigo-300 font-mono text-xs truncate max-w-xs">
                            {c.purl || "—"}
                          </td>
                        </tr>
                      ))}
