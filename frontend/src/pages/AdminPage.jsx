...
const RoleBadge = ({ role }) => (
  ...
  >
    <span
      className={`w-1.5 h-1.5 rounded-full ${role === "admin" ? "bg-amber-400" : "bg-indigo-400"}`}
    />
...
const AdminPage = () => {
  ...
  const handleDeleteUser = async (userId, userName) => {
    ...
    try {
      ...
      setUsers((currentUsers) =>
        currentUsers.filter((currentUser) => currentUser._id !== userId),
      );
  ...
  const filteredUsers = users.filter(
    (u) =>
      ...
      getOrganizationName(u.organization)
        .toLowerCase()
      ...
      <table className="w-full">
        <thead>
          <tr className="border-b border-white/8">
            <th className="text-left text-xs font-semibold text-slate-400 uppercase tracking-wider px-3 sm:px-6 py-3 sm:py-4">
              #
            </th>
            <th className="text-left text-xs font-semibold text-slate-400 uppercase tracking-wider px-3 sm:px-6 py-3 sm:py-4">
              User
            ...
            {showDelete && (
              <th className="text-right text-xs font-semibold text-slate-400 uppercase tracking-wider px-3 sm:px-4 py-3 sm:py-4">
                Action
              </th>
            )}
          </tr>
          ...
          {list.length === 0 ? (
            ...
            list.map((u, idx) => (
              ...
              >
                ...
                <td className="px-3 sm:px-4 py-3 sm:py-4 text-sm text-slate-300 whitespace-nowrap">
                  {getOrganizationName(u.organization)}
                </td>
                <td className="px-3 sm:px-4 py-3 sm:py-4 text-sm text-slate-300 whitespace-nowrap">
                  {u.country || "—"}
                </td>
                <td className="px-3 sm:px-4 py-3 sm:py-4 text-xs text-slate-400 whitespace-nowrap">
                ...
                {showDelete && (
                  <td className="px-3 sm:px-4 py-3 sm:py-4 text-right">
                    <button
                      type="button"
                      onClick={() =>
                        handleDeleteUser(u._id, u.name || "this user")
                      }
  ...
  return (
    ...
    <div
      className={`min-h-[calc(100vh-64px)] ${isDark ? "bg-[#0f0f1a]" : "bg-[#f1f5f9]"} p-4 sm:p-6 md:p-8 lg:p-10`}
    >
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
          ...
          <button
            onClick={fetchUsers}
            disabled={loading}
            className="inline-flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/10 text-slate-300 rounded-xl text-sm font-medium transition-all cursor-pointer"
          >
        ...
        <div className="mb-6 sm:mb-8 bg-[#13131f]/80 backdrop-blur-xl border border-white/8 rounded-2xl overflow-hidden">
          ...
          {loading ? (
            <div className="flex items-center justify-center py-20 relative min-h-[300px]">
