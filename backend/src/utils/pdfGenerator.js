...
const infoBox = (doc, text, y, color = C.brand, bg = '#eef2ff') => {
    ...
    doc.fillColor(C.grayDark).font('Helvetica').fontSize(8.5).lineGap(2)
...
const generateScanPDF = (scan, report) => {
    ...
    doc.save()
       .strokeColor(C.brand).opacity(0.07).lineWidth(80)
       .moveTo(PAGE_W, 0)
       .quadraticCurveTo(PAGE_W - 200, PAGE_H * 0.5, PAGE_W, PAGE_H)
       .stroke()
       .restore();
    ...
    ], tocSections.map((title, i) => [i + 1, title, '—']), y);
    ...
    y = dataTable(doc, [
        ...
        { label: 'Priority',      width: CW - 360, align: 'center', bold: true,
          color: (v) => v === 'Critical' ? C.red : v === 'High' ? C.orange : C.yellowDark },
    ], [
    ...
    const sevCards = [
        ...
        { label: 'MEDIUM',   count: vulnMedium,   textColor: C.yellowDark,bg: C.yellowBg },
    ...
    sevCards.forEach((card, i) => {
        ...
        doc.fillColor(card.textColor).font('Helvetica-Bold').fontSize(7.5)
           .text(card.label, cx + 10, y + 11, { width: cardW - 14, lineBreak: false });
        doc.fillColor(card.textColor).font('Helvetica-Bold').fontSize(28)
           .text(String(card.count), cx + 10, y + 28, { width: cardW - 14, lineBreak: false });
    });
    y += cardH + 16;
    ...
    if (y + 60 > BODY_BOT) { y = addPage(doc); }
    y = subHeading(doc, 'Component Summary', y);
    y = kvTable(doc, [
        ['Total Components',          String(components.length || summary.totalComponents || 0)],
        ['Vulnerable Components',     String(components.filter(c => c.vulnerable).length)],
        ['Outdated Libraries',        String(components.filter(c => c.outdated).length)],
        ['Components with Exploits',  String(components.filter(c => c.exploit).length)],
    ], y);
    ...
    y = dataTable(doc, [
        ...
        ['MEDIUM',   vulnMedium,   'Notable risk; address within standard patch cycle.'],
    ...
    y = dataTable(doc, [
        { label: 'Phase',       width: 42,  align: 'center', bold: true, color: C.brand },
...
for (let i = 0; i < 8; i++) {
    doc.moveTo(0, PAGE_H - 80 + i * 5)
       .bezierCurveTo(
           140, PAGE_H - 120 + i * 5,
           420, PAGE_H - 20 + i * 5,
           PAGE_W, PAGE_H - 80 + i * 5
       )
       .stroke();
}
doc.restore();
...
doc.fillColor(C.grayDark)
   ...
   .text(
      ...
      'RNT Infosec LLP builds a broader ecosystem of platforms designed to strengthen an ' +
...
//     // Centred thank-you box
//     const boxY = y + 30;
//     const boxH = 140;
//     fillRect(doc, M, boxY, CW, boxH, '#eef2ff');
//     strokeRect(doc, M, boxY, CW, boxH, C.brand, 1);
//     fillRect(doc, M, boxY, 6, boxH, C.brand);

//     doc.fillColor(C.brand).font('Helvetica-Bold').fontSize(22)
//        .text('Thank You', M + 6, boxY + 22, { width: CW - 6, align: 'center' });
...
//            M + 16, boxY + 58,
//            { width: CW - 30, align: 'center' },
//        );

//     doc.fillColor(C.grayMid).font('Helvetica').fontSize(8)
//        .text(
...
doc.fillColor(C.grayDark)
   ...
   .text(
      ...
      'security engagement. RNT Infosec LLP remains committed to helping organizations ' +
      ...
      'We appreciate your trust in RNT Infosec and look forward to supporting your continued ' +
...
doc.fillColor(C.grayMid)
   ...
   .text(
      ...
      'is prohibited without prior written consent from RNT Infosec LLP.',
      M + 20,
      PAGE_H - 165,
      {
         width: 250,
...
doc.save();
doc.circle(PAGE_W - 110, PAGE_H - 95, 45)
   .fill('#dbeafe');
...
for (let i = 0; i < 8; i++) {
    doc.moveTo(0, PAGE_H - 70 + i * 4)
       .bezierCurveTo(
           120, PAGE_H - 110 + i * 4,
           420, PAGE_H - 20 + i * 4,
           PAGE_W, PAGE_H - 70 + i * 4
       )
       .stroke();
}
doc.restore();
